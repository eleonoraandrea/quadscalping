...
type Params struct {
	...
	// ---- parametri avanzati per migliorare profitti ----
	TrailingStopATR   float64 // trailing stop ATR multiplier (0 = disabilitato)
	DynamicTP         bool    // adatta TP in base alla volatilità
	VolatilityWindow  int     // finestra per calcolare volatilità (default 20)
	MaxHoldBars       int     // massimo numero di barre in posizione (0 = illimitato)
	ATRPeriod         int     // periodo ATR (default 14)
	AdaptiveExitMult  float64 // moltiplicatore per exit anticipata se volatilità alta
}
...
type Series struct {
	...
	VolAvg20             []float64
	Volatility           []float64 // misura volatilità per TP dinamico
}
...
func Compute(candles []market.Candle, p Params) Series {
	...
	// calcolo ATR con periodo dinamico
	atrPeriod := p.ATRPeriod
	if atrPeriod <= 0 {
		atrPeriod = 14
	}
	
	// calcolo volatilità per TP dinamico (standard deviation dei rendimenti)
	volWindow := p.VolatilityWindow
	if volWindow <= 0 {
		volWindow = 20
	}
	...
	for i := volWindow; i < n; i++ {
		...
		mean /= float64(volWindow)
		sum /= float64(volWindow)
		vol := math.Sqrt(mean - sum*sum)
		volatility[i] = vol * 100 // in percentuale
	}
...
func (s *Series) Evaluate(candles []market.Candle, p Params, i int) Signal {
	...
	// calcolo TP dinamico basato sulla volatilità
	tp1Multiplier := p.TP1R
	if p.DynamicTP && i < len(s.Volatility) && s.Volatility[i] > 0 {
		// se volatilità alta, aumento TP per catturare movimenti più ampi
		volFactor := 1.0 + (s.Volatility[i] / 50.0) // normalizzato
		if volFactor > 2.0 {
			volFactor = 2.0
		}
		tp1Multiplier *= volFactor
	}
