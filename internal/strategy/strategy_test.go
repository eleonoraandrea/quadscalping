...
func TestBuyEntryOnQuadHook(t *testing.T) {
	...
	// con DynamicTP attivo, il TP1 puo' essere maggiore di entry + risk*TP1R
	expectedTP1Min := sig.EntryPrice + risk*p.TP1R
	if sig.TP1 < expectedTP1Min {
		t.Errorf("tp1 %v troppo basso (minimo atteso %v)", sig.TP1, expectedTP1Min)
	}
